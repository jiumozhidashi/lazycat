...
def uploadfile(request):
        ...
        for station_cell in st_col:
            if station_cell.value == "单价(元)" or station_cell.value is None:
                ...
                for ma_row in oldws.iter_rows(min_row=material_start_row, max_row=read_max_row,
                    ...
                    write_col = 1
                    material_qty = oldws.cell(
                        row=read_row, column=read_col).value
                    if (ma_row[3].value == '甲供')and (material_qty != 0) and (material_qty is not None):
                        for r in ma_row[0:3]:
                            newws.cell(row=write_row,
                                       column=write_col, value=r.value)
                            write_col += 1
                        newws.cell(row=write_row, column=write_col,
                                   value=material_qty)
                        newws.cell(row=write_row, column=write_col +
                                   1, value=station_cell.value)
                        write_row += 1
                    read_row += 1
                read_col += 1
    wb.save('static/downloads/%s' % obj1.name)
    df_ysb = pd.read_excel('static/downloads/%s' %
                           obj1.name, sheet_name='formatsheet')
    ...
    df_material_use = pd.merge(
        df_ysb, df_task_list[['工程名称', 'MIS任务号']], on='工程名称', how='outer')
    df_material_use = df_material_use.round({'数量': 3})
    df_material_name = df_material_use['物料名称'].str.cat(
        df_material_use['规格'], na_rep='').str.strip()
    df_material_use = pd.concat(
        [df_material_name, df_material_use[['单位', '数量', '工程名称', 'MIS任务号']]], axis=1)
    print(df_material_use)
    df_material_name = df_material_name.drop_duplicates(
    ...
    df_material_mis_name = df_material_mis_name.drop_duplicates(
        keep='first', inplace=False)
    df_material_mis_name = df_material_mis_name.reset_index()['备注']
    df_rules = pd.read_excel('static/rules.xlsx')
    df_mis = pd.merge(df_mis, df_rules, left_on='备注',
                      right_on='MIS物料名称', how='left')
    df_mis_grouped = df_mis.groupby(['演算表物料名称', '任务'], as_index=False).sum()
    df_mis_grouped = df_mis_grouped[['演算表物料名称', '任务', '数量']]
    df_mis_grouped.columns=['物料名称','MIS任务号','数量']
    df_material_use_grouped = df_material_use.groupby(
        ['物料名称', 'MIS任务号'], as_index=False).sum()
    df_material_use_grouped['数量'] = -df_material_use_grouped['数量']
    df_grouped = pd.concat ( [ df_mis_grouped,df_material_use_grouped])
    df_grouped = df_grouped.groupby(['物料名称', 'MIS任务号'], as_index=False).sum()
    print(df_grouped)
    # df_merge = pd.merge(df_mis, df_material_use, left_on=['演算表物料名称', '任务'], right_on=['物料名称', 'MIS任务号'],
    #                  how='outer')
    df_grouped.to_excel("static/df_grouped.xlsx",
                            sheet_name="01", index=False, header=True)
